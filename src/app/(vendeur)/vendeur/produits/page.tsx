  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-titre-sm text-nile-800 sm:text-titre-md">Mes produits</h1>
          ...
          {produits.map((p) => (
            <Link key={p.id} href={`/vendeur/produits/${p.id}`} className="block">
              <Carte className="flex items-center gap-3 p-3 transition hover:shadow-flottant">
                <Vignette url={p.images[0]?.url} alt="" sizes="56px" className="h-14 w-14 shrink-0 rounded" />
